...
import { Swords, Users, Zap, Shield, Crown, Coins, Loader2, Heart, Hash } from 'lucide-react'
...
type PvpViewState = 'select' | 'browse' | 'join'
...
export default function BattlePage() {
  ...
  // PVP State
  const [pvpView, setPvpView] = useState<PvpViewState>('select')
  ...
  const handleCreateRoom = async () => {
    ...
    if (beasts.length === 0) {
      setPvpError('You need at least one beast to battle. Visit your inventory to get started!')
      return
    }
    ...
    try {
      ...
      setPvpError(error.message || 'Failed to create room')
    } finally {
  ...
  const handleJoinRoom = async (roomCode: string) => {
    ...
    // Client-side validation: Check if user has beasts
    if (beasts.length === 0) {
      setPvpError('You need at least one beast to join a battle. Visit your inventory to get started!')
      return
    }
    ...
    if (trimmedCode.length !== 6) {
      setPvpError('Room code must be exactly 6 characters.')
      return
    }
    ...
    if (!validChars.test(trimmedCode)) {
      setPvpError('Invalid room code format. Please check the code and try again.')
      return
    }
    ...
    try {
      ...
      setPvpError(error.message || 'Failed to join room')
    } finally {
      setIsPvpLoading(false)
    }
  }
  ...
  const renderBeastList = () => {
    ...
    return beasts.map((beast) => {
      ...
      return (
        <Card
          key={beast.id}
          className={`cursor-pointer transition-all duration-300 hover:scale-105 hover:shadow-lg active:scale-95 animate-in slide-in-from-left ${selectedBeast === beast.id ? 'ring-4 ring-primary shadow-primary/50' : ''
            }`}
  ...
  const renderPvpBrowseView = () => (
    <Card className="max-w-4xl mx-auto">
      <CardHeader>
        <div className="flex justify-between items-center">
          <CardTitle className="text-2xl">Available Rooms</CardTitle>
          <Button variant="outline" onClick={() => setPvpView('select')}>
            Back
          </Button>
        </div>
      </CardHeader>
      <CardContent>
        {availableRooms.length === 0 ? (
          <div className="text-center py-12">
            ...
            <Button onClick={() => setPvpView('select')}>
              Create a Room
            </Button>
          </div>
        ) : (
      ...
      <CardContent>
        <div className="space-y-4">
          ...
          <Button
            size="lg"
            className="w-full"
            disabled={roomCodeInput.length !== 6 || isPvpLoading}
      ...
      <main className="container mx-auto px-4 py-8 animate-in fade-in duration-700">
        ...
        <Tabs defaultValue="pvp" className="w-full">
          ...
          <TabsContent value="pvp" className="mt-6">
            ...
            {pvpView === 'select' && renderPvpSelectView()}
            {pvpView === 'browse' && renderPvpBrowseView()}
            {pvpView === 'join' && renderPvpJoinView()}
          </TabsContent>
        </Tabs>
